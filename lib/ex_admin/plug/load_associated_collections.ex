defmodule ExAdmin.Plug.LoadAssociatedCollections do
  @moduledoc """
  Fetch associated collections for a given model
  
  ## Examples

      iex> plug ExAdmin.Plug.LoadAssociaedCollections
  """
  ...
  def call(conn, opts) do
    ...
    repo = opts[:repo] || ex_admin[:repo] || raise("repo required")
    ...
    case conn.assigns[assigns_key] do
      ...
      resource -> 
        ...
        ex_admin = conn.assigns[:ex_admin] || %{}
        assign conn, :ex_admin, Map.put(ex_admin, :associations, assocs)
    end
  end
