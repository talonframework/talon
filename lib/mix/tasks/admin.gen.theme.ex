defmodule Mix.Tasks.Admin.Gen.Theme do
  @moduledoc """
  Create a new ExAdmin theme.

      mix admin.gen.theme theme target_name

  Copies `theme` to a new `theme_name`. Creates the following files:

  * web/views/admin/target_name path
  * web/templates/admin/target_name/generators path
    * edit.html.eex
    * form.html.eex
    * index.html.eex
    * new.html.eex
  ...
  * --verbose -- Print extra information
  ...
  def gen_layout_view(config) do
    ...
    theme = config.theme
    view_path = Path.join([web_path(), "views", "admin", config.target_name])
    unless config.dry_run do
      File.mkdir_p! view_path
      copy_from paths(),
        "priv/templates/admin.gen.theme/#{theme}/views", view_path, binding, [
  ...
  defp gen_layout_templates(config) do
    ...
    theme = config.theme
    template_path = Path.join([web_path(), "templates", "admin", config.theme, "layout"])
    unless config.dry_run do
      File.mkdir_p! template_path
      copy_from paths(),
        "priv/templates/admin.gen.theme/#{theme}/templates/layout", template_path, binding, [
  ...
  defp gen_generators(config) do
    binding = Kernel.binding() ++ [base: config.base, target_name: config.target_name, target_module: config.target_module]
    theme = config.theme
    template_path = Path.join([web_path(), "templates", "admin", config.theme, "generators"])
    unless config.dry_run do
      File.mkdir_p! template_path
      copy_from paths(),
        "priv/templates/admin.gen.theme/#{theme}/templates/generators", template_path, binding, [
  ...
  defp get_available_themes do
    :ex_admin
    |> Application.app_dir("priv/templates/admin.gen.theme/*")
    |> Path.wildcard()
